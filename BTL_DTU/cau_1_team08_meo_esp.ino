...
const char* default_mqtt_server = "45.76.163.233";
const char* default_mqtt_port = "1883";
const char* device_id = "team08";
...
//const int DHTPIN = D2;      //Đọc dữ liệu từ DHT11 ở chân A0 trên ESP
...
void callback(char* topic, byte* payload, unsigned int length) {
	...
	Serial.println("Cac Gia tri nhan duoc :");
	for (int i = 0; i < index1; i++)
	{
		Serial.println(String(A[i][0]) + " " + String(A[i][1]));
	}
	Serial.println("-----------------------------------");
...
//////////////////////////////PUB/////////////////
void send_data() {
	int D51 = digitalRead(D2);
	int D61 = digitalRead(D6);
	int D71 = digitalRead(D7);
	int D81 = digitalRead(D8);
	...
	int U4 = Virtual_U4();
	int U5 = Virtual_U5();
	String pubString = String(D51) + ';' + String(D61) + ';' + String(D71) + ';' + String(D81) + ';' + String(A0) + ';' + String(U1) + ';' + String(U2) + ';' + String(U3) + ';' + String(U4) + ';' + String(U5);
...
void gettemperature() {
	...
	if (currentMillis - previousMillis >= cnt_sensor) {
		...
		if (temp_c <= 31) digitalWrite(5, LOW);
